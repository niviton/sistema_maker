...
# ===== ESP32 HTTP (mantém seu comportamento) =====
ESP_IP = "192.168.0.53"
...
# Remove o controlar_led_esp original e substitui por gravação no arquivo
def controlar_led_arduino(on: bool):
    cmd = "1" if on else "0"
    try:
        with open(COMANDO_FILE, "w") as f:
            f.write(cmd)
        app.logger.info(f"Comando serial escrito: {cmd}")
    except Exception as e:
...
if not os.path.exists(ESTADO_LAB_TXT):
    open(ESTADO_LAB_TXT,'w').write('FECHADO')
if not os.path.exists(SHEETS_BUFFER_JSON):
    open(SHEETS_BUFFER_JSON,'w').write(json.dumps([]))
# Arquivo de comando serial
...
def set_estado_lab(novo):
    open(ESTADO_LAB_TXT,'w').write(novo.strip().upper())
...
def enqueue_sheets(params):
    buf = load_buffer(); buf.append(params); save_buffer(buf)
def try_flush_buffer():
    buf = load_buffer(); new_buf = []
    for params in buf:
        try:
            r = requests.get(SHEETS_WEBAPP_URL, params=params, timeout=5)
            r.raise_for_status()
            app.logger.info(f"[Sheets] enviado: {params}")
        except Exception as e:
            app.logger.warning(f"[Sheets] falha buffer {params}: {e}")
...
def visitante():
    ...
    if request.method=='POST':
        ...
        abrir_porta_esp()
        controlar_led_arduino(True)
        threading.Timer(5, lambda: controlar_led_arduino(False)).start()
...
def bolsista():
    if request.method=='POST':
        ...
        enqueue_sheets({
            'nome':nome,'id':matricula,
            'acao':'PONTO','datahora':datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        })
        return redirect(url_for('bolsista_success',
            nome=nome, matricula=matricula, estado=get_estado_lab()))
