...
def abrir_porta():
    """Tenta ESP32; se falhar, usa GPIO local."""
    try:
        resp = requests.get(f"{ESP_BASE_URL}/open", timeout=1)
...
def enviar_para_sheets(params):
    try:
        ...
        app.logger.warning(f"[Sheets] falha imediata, enfileirando: {e}")
        enqueue_sheets(params)
...
def visitante():
    ...
    if request.method=='POST':
        ...
        abrir_porta()
        controlar_led(True)
        threading.Timer(0.1, lambda: controlar_led(False)).start()
